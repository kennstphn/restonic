...
| Email: chad@s-go.net								  |
| Descriptions: modifications were to addd results    |
...
jQuery(document).ready(function () {
  ...
  function placeMarker (address, name, street, city, state, zip, phone, lt, lg, url)
  {
    ...
    if (distance <= distanceConfig || zipConfig == zip )
    {
      ...
      var contentString = '<div style="color: #000000;">Distance: ' + distance + ' miles<br /><strong>' + name + '</strong><br />' + street + '<br />' + city + ', ' + state + ' ' + zip  + '<br /><a href="http://maps.google.com/?daddr=' + street + '+' + city + '+' + state + '+' + zip + '" target="_blank">Get Directions</a><br /><a href="' + url + '">' + url + '</a></div>';
  ...
  function geocodeAndPlaceMarker (address, name, street, city, state, zip, phone)
  {
    ...
    geocoder.geocode({'address': address}, function (results, status) {
      if (status == google.maps.GeocoderStatus.OK)
      {
        ...
        // var point1 = new google.maps.LatLng(originalLatLng);
        // var point2 = new google.maps.LatLng(myLatlng);
